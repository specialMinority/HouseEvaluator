...
작성 기준: 2026-09-13. [render.yaml](../render.yaml)은 재현용 Blueprint 설정이며, 아래 1~5절은 구성·운영 절차다. **첫 실제 서비스는 공개 Git 저장소를 연결해 수동 생성했다.** Blueprint는 사용하지 않았다. 6~7절은 과거 배포 기록이며, **현재 PC 작업자 구성과 공개 검증 결과는 8절**에 기록한다.
...
| 원문 조회 실행 위치 | `HOUSE_EVALUATOR_EXECUTION_MODE=worker`; 운영자 PC에서 처리 |
| 자동검색 출처 | `HOUSE_EVALUATOR_SEARCH_SOURCE=suumo`; PC 작업자와 같은 고정 설정 |
| URL 자동입력 출처 | `HOUSE_EVALUATOR_IMPORT_SOURCES=chintai,yahoo_realestate`; PC 작업자와 같은 설정 |
| 작업자 인증 | `HOUSE_EVALUATOR_WORKER_TOKEN`; 사용자 접속 코드와 별도의 비밀값 |
...
앱 자체의 검색 작업도 메모리에만 있고 기본 유효기간은 15분이다. 현재 작업자 모드는 대기·실행 합계 최대 5개, 원문 조회는 동시에 1개다. Render가 재시작하면 작업 ID가 유효하지 않게 된다. 이미 화면에 받은 목록은 해당 탭 메모리에 남아 비교할 수 있지만 새로고침하면 입력·목록·접속 코드가 지워진다. 저장·복구 기능이 있는 서비스로 안내하지 않는다.

원문 조회에는 운영자 PC의 전원·현재 사용자 로그인·인터넷 연결이 필요하다. 현재 사용자 로그인 시 자동 시작하는 작업을 설치하지만 절전·로그아웃은 막지 않는다. 작업자의 확인 요청도 Render 수신 트래픽이므로 무료 시간 한도를 소비한다. 설정·중단·수명 제한은 [REMOTE_WORKER.md](REMOTE_WORKER.md)를 따른다.
...
최초 배포 당시 재평가 결론은 `degraded_public_search`였다. 기본 HTTP·인증·가상 비교 검사는 통과했지만 당시 실제 자동검색은 503·0건이었다. 이 응답만으로 사이트 장애인지 호스팅 환경의 제한인지 구분할 수 없다. 400 응답이 어느 계층에서 발생했는지도 상태 코드만으로 확정하지 않는다.
...
당시 원격 상세 증거는 로컬 `.runtime/render-yahoo-acceptance.json`에만 보관한다. 이 절의 직접 클라우드 조회 실패 기록은 유지하며, 이후 PC 작업자를 통한 공개 검증과 구분한다.
...
## 8. 현재 운영 상태: Render와 PC 작업자

2026-09-13 11:06~11:08 KST에 [실제 공개 서비스](https://houseevaluator-personal.onrender.com)에서 URL 조건 자동입력 → 자동검색 → 비교를 확인했다. Render가 화면·인증·작업 중계를 제공하고, 사전에 지정한 PC 작업자가 원문을 조회한다. 실패한 클라우드 요청을 다른 호스트에서 자동 재시도하는 방식은 아니다.

| 항목 | 확인 내용 |
| --- | --- |
| 실행 코드 | `af7231de6c0c3660ea2afecee466d9ea7594c4e1` |
| Render 배포 | `dep-daj09o8ae00c7385ml40`, Live 11:05:55 KST |
| 실행 설정 | `worker` 모드, SUUMO 검색, CHINTAI·Yahoo! 부동산 URL 입력 지원 |
| 실제 URL 입력 | Yahoo! 부동산의 상세 조건이 공개 사이트 입력폼에 반영됨 |
| 실제 자동검색 | 목록 3페이지에서 광고 60개 반환, 상세 정보 22개 반영 |
| 실제 비교 화면 | 비교 광고 4개와 대상의 가격 막대, 표시 표본 내 순위·원문 링크 확인 |
| 코드 검증 | 시간 차이 대응 관련 회귀 153개 통과; [GitHub Actions 성공](https://github.com/specialMinority/HouseEvaluator/actions/runs/34732146457) |
| 작업자 중단·복구 | 전용 예약 작업·식별 확인한 프로세스 중단 후 30초 경과 → 연결 재확인으로 오프라인 안내·검색/URL 버튼 비활성 확인. 재시작 후 온라인·두 버튼 활성 복구. 입력·기존 그래프 유지, 추가 원문 요청 0회 |

PC와 서버의 작은 시각 차이로 최초 작업 결과가 거절된 문제는 서버 UTC 앵커와 단조 시계를 이용한 최대 5초 대기로 처리했다. 원문의 조회 시각과 시스템 시계는 변경하지 않으며, 한도를 넘으면 명확한 오류를 반환한다. 중단·복구 검증을 마친 작업자는 현재 실행 중 상태로 복구했다.

**PC 전원·현재 사용자 로그인·인터넷 연결이 유지돼야 조회할 수 있다.** 현재 사용자 로그인 시 자동 시작하도록 구성하지만 절전·로그아웃·통신 장애를 막지 않는다. 원문 정책·접근 제한·HTML 변경에도 영향을 받는다. Render 단독으로 원문에 직접 접속하는 구성의 복구는 확인되지 않았으며, 6~7절의 실패는 과거 사실로 보존한다. 설정과 복구 절차는 [REMOTE_WORKER.md](REMOTE_WORKER.md)를 참고한다.

현재는 공유 접속 코드를 사용하는 무료 소규모 미리보기다. 이번 확인은 해당 조회와 비교 흐름의 성공이며 상시 운영, 전체 시장 가격, 현재 공실을 보장하지 않는다. 실제 매물 조건·상세 링크·이름·비밀값은 공개 배포 문서에 기록하지 않는다.

## 9. PC 조회 작업자 격리 전환

2026-09-13 보안 보완으로 Windows에서 직접 실행하던 Python 작업자를 전용 Docker 구성으로 전환한다. PC 전원·로그인·인터넷 외에 Docker Desktop 실행이 필요하다. 네트워크 없는 작업자와 고정 HTTPS 통신 통로를 분리하고 파일·자원·요청 총량을 제한한다. 현재 검증과 잔여 위험은 [PC_WORKER_SECURITY.md](PC_WORKER_SECURITY.md), 시작·중단은 [REMOTE_WORKER.md](REMOTE_WORKER.md)를 따른다. 과거 직접 실행 방식의 중단 명령을 그대로 사용하지 않는다.
