...
현재 기본 화면은 **공급사 없이, 무료로 로컬에서 사용하는 개인 비교**입니다. 집의 조건을 입력하면 SUUMO 공개 매물을 검색하고, 검색 실패나 정보 누락 시 직접 입력·수정하여 비교를 계속합니다. 선택한 광고 표본의 월세+관리비 중앙값과 차이를 보여주며, 시장 전체의 적정 가격이나 현재 공실을 보증하지 않습니다. 사이트 정책·HTML 변경에는 영향을 받으며, 거절된 접근을 우회하지 않습니다.

공급 파일/HTTPS JSON 피드, 시장 검증, 접속 제어, 백업·복구 등 기존 공급형 기능도 보존했습니다. 실제 공급 계약·시장 검증·외부 공개는 아직 완료되지 않았으며, **개인 비교의 선행 조건은 아닙니다**. 사용법과 제한은 [PERSONAL_COMPARISON.md](docs/PERSONAL_COMPARISON.md)를 참고하세요.
...
1. 도시·구·일본어 역명과 대상 매물 조건을 입력합니다.
...
검색은 공식 노선·역 선택 폼을 확인한 역 중심 탐색과 근접 면적 우선 탐색을 사용합니다. 45초 안에서 최대 3개 목록과 24개 상세를 확인하고, 한 건물의 다수 광고가 후보를 독점하지 않게 나눠 읽습니다. 검색 범위·광고 수·잠정 건물 수·상세 확인 수·종료 사유를 함께 표시하며, 이번 조회에서 비교군을 찾지 못한 것을 주변 매물 전체의 부재로 표현하지 않습니다.
...
| `SUUMO_LIVE` | Docker와 CI는 `0`; v2는 값과 관계없이 포털 조회 경로를 사용하지 않음 |
