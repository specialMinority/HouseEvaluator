"""Bounded reads of Yahoo! 부동산's public listing pages, with observed navigation.
...
from .yahoo_discovery import build_search_url, checked_url, municipality_url, next_page, resolve_layout_theme, resolve_station
from .yahoo_html import parse_search
...
def search(subject, *, fetcher=None):
    ...
    started = _timestamp()
    report = {
        'source_id': 'yahoo_realestate', 'status': 'unavailable',
        'message': 'Yahoo! 부동산 공개 검색에 연결하지 못했습니다.',
        'search_url': None, 'search_urls': [], 'fetched_at': None,
        'listing_count': 0, 'search_page_count': 0, 'station_page_count': 0, 'layout_theme_page_count': 0,
    ...
    result = {
        ...
        'scope': 'public_search_sample', 'price_filter_applied': False, 'server_condition_filters_applied': False,
        'notice': 'Yahoo! 부동산 공개 목록 최대 3페이지의 일부입니다. 모집 상태와 조건은 각 매물 원문에서 확인하세요.',
    }
    try:
    ...
    try:
        robots = read('https://realestate.yahoo.co.jp/robots.txt', robots=True)
        ...
        while url and report['search_page_count'] < MAX_SEARCH_PAGES:
            ...
            if '/theme/' in url:
                report['layout_theme_page_count'] += 1
                result['server_condition_filters_applied'] = True
            report.update(status='ok', message='Yahoo! 부동산 공개 목록에서 비교 후보를 읽었습니다. 조건과 모집 상태를 원문에서 확인하세요.')
            ...
            if station_url is None:
                ...
                if resolved:
                    ...
                    if destination not in attempted:
                        ...
                        continue
            # Layout themes are ordinary observed navigation links. The dynamic
            # search form's /rent/search/? URL is not used or rearranged.
            themed = resolve_layout_theme(text, subject, base_url=url)
            url = themed if themed and themed not in attempted else next_page(text, url, max_page=MAX_SEARCH_PAGES)
        ...
        try:
            ...
            theme_label = ('1K·1DK 함께' if subject['layout'] in ('1K', '1DK') else subject['layout'])
            layout_scope = ('원문 평면 테마 ' + theme_label if report['layout_theme_page_count'] else '원문 평면 필터 미적용')
            report['area_scope_label'] = layout_scope + ' · 원문 면적 필터 미적용 · 반환 후보만 같은 역·평면, 대상 전용면적 ±20%'
